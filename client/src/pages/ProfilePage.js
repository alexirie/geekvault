...
export default function ProfilePage() {
  ...
  const changeLanguage = (lng) => {
    ...
    localStorage.setItem("lang", lng); // persistente
        ...
        <div className="mt-8 bg-[#1b2838] rounded-2xl shadow-[0_0_10px_rgba(102,192,244,0.3)] divide-y divide-[#2a3b52]">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-[#c7d5e0]">{t("perfil.idioma")}</span>
            </div>
            <span className="text-[#66c0f4]">{showLanguages ? "▲" : "›"}</span>
          </button>

          {/* Submenú idiomas */}
          {showLanguages && (
  <div className="bg-[#141a22] rounded-b-2xl overflow-hidden">
    {[
      ...
      return (
        <button
          ...
          className={`
            ...
            transition-all
            ${isActive
              ? "bg-[#1b2838] text-[#66c0f4] border-l-4 border-[#66c0f4] shadow-[inset_0_0_8px_rgba(102,192,244,0.4)]"
              : "text-[#9fb6cc] hover:bg-[#1f2a38]"
            }
          `}
        >
          <span>{lang.label}</span>
          {isActive && <span className="text-[#66c0f4]">●</span>}
        </button>
      );
    })}
  </div>
